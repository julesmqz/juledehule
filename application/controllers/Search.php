...
class Search extends JH_Controller
{
    ...
    public function categoria($cat, $page = 1)
    {
        $this->_preparePaginationData($page, '/busqueda/categoria/' . $cat, $this->mpost->getTotalPagesByCat($cat), true);

        $posts = $this->mpost->searchByCat($cat, $page);
        foreach ($posts as &$post) {
            $co = $post['created_on'];
            $uo = $post['updated_on'];
            $post['created_on'] = date('d/M/Y', strtotime($co));
        ...
        $this->_contentData = array(
            'posts' => $posts,
            'search_value' => 'categoria:' . $cat,
        );

        $this->_shareMetadata[] = array(
            'url' => 'http://www.juledehule.com.mx/busqueda/categoria/' . $cat,
            'type' => 'search',
            'description' => 'Resultados de la búsqueda por categoría ' . $cat,
            'image' => 'http://assets.juledehule.com.mx/img/logo_vertical.png',
            'title' => 'Resultados de la búsqueda por categoría ' . $cat,
        );
